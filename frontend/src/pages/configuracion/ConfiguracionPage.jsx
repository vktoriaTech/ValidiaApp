...
function SectoresTab() {
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      setFormError(err.response?.data?.detail || 'No fue posible guardar el sector.')
    } finally {
      setSaving(false)
    }
  }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'No fue posible eliminar el sector.')
      setDeleteTarget(null)
    } finally {
      setDeleting(false)
    }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">
          Catálogo global de sectores económicos para clasificar clientes.
        </p>
      ...
      {deleteTarget && (
        <ConfirmModal
          ...
          message={`¿Estás seguro de que deseas eliminar el sector "${deleteTarget.name}"? Los clientes asignados quedarán sin sector.`}
          onCancel={() => setDeleteTarget(null)}
          onConfirm={confirmDelete}
          confirming={deleting}
        />
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card>
        ...
        {activeTab === 'sectores' && <SectoresTab />}
